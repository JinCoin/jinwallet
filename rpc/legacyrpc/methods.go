...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by jinwallet due to
...
// unsupported handles a standard jincoind RPC request which is
// unsupported by jinwallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &btcjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by jinwallet",
	}
}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*jinutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := jinutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]jinutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := jinutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
// TODO: finish this to match jincoind by writing the dump to a file.
...
// information about the current state of jinwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to jind for all of the information in this command known
	// by them.
...
func decodeAddress(s string, params *chaincfg.Params) (jinutil.Address, error) {
	addr, err := jinutil.DecodeAddress(s, params)
	if err != nil {
...
// appear in the blockchain, or any tx that has arrived in the jind mempool).
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := jinutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
...
//
// Note: jincoind allows specifying the account as an optional parameter,
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		debitTotal  jinutil.Amount
		creditTotal jinutil.Amount // Excludes change
		fee         jinutil.Amount
		feeF64      float64
	)
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// jind returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to jind.  Therefore, create a
	...
	// texts in a non-internal jind package.
...
// and amounts.
func makeOutputs(pairs map[string]jinutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := jinutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
func sendPairs(w *wallet.Wallet, amounts map[string]jinutil.Amount,
	account uint32, minconf int32, feeSatPerKb jinutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := jinutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]jinutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]jinutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := jinutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func sendToAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	// sendtoaddress always spends from the default account, this matches jincoind
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// TODO: really we probably should look these up with jind anyway to
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := jinutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	// querying jind with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*jinutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*jinutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := jinutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := jinutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	case *jinutil.AddressPubKeyHash: // ok
		return bytes.Equal(jinutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *jinutil.AddressPubKey: // ok
