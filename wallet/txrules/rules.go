...
const DefaultRelayFeePerKb jinutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb jinutil.Amount) jinutil.Amount {
	...
	relayFee := jinutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount jinutil.Amount, scriptSize int, relayFeePerKb jinutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb jinutil.Amount) bool {
	...
	return IsDustAmount(jinutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb jinutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > jinutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb jinutil.Amount, txSerializeSize int) jinutil.Amount {
	fee := relayFeePerKb * jinutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > jinutil.MaxSatoshi {
		fee = jinutil.MaxSatoshi
	}
