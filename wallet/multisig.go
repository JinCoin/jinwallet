...
func (w *Wallet) MakeMultiSigScript(addrs []jinutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*jinutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*jinutil.AddressScriptHash, error) {
	var p2shAddr *jinutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = jinutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*jinutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
