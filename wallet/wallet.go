...
import (
	...
	"github.com/JinCoin/jinwallet/wallet/txauthor"
	"github.com/JinCoin/jinwallet/wallet/txrules"
...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]jinutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	var addrs []jinutil.Address
	err := w.Manager.ForEachActiveAddress(addrmgrNs, func(addr jinutil.Address) error {
...
func (w *Wallet) CreateSimpleTx(account uint32, outputs []*wire.TxOut,
	minconf int32, satPerKb jinutil.Amount) (*txauthor.AuthoredTx, error) {
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []jinutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (jinutil.Amount, error) {
...
type Balances struct {
	Total          jinutil.Amount
	Spendable      jinutil.Amount
	ImmatureReward jinutil.Amount
}
...
// CurrentAddress gets the most recently requested Jincoin payment address
// from a wallet.  If the address has already been used (there is at least
// one transaction spending to it in the blockchain or jind mempool), the next
// chained address is returned.
func (w *Wallet) CurrentAddress(account uint32) (jinutil.Address, error) {
	var addr jinutil.Address
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) PubKeyForAddress(a jinutil.Address) (*btcec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a jinutil.Address) (*btcec.PrivateKey, error) {
...
func (w *Wallet) HaveAddress(a jinutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a jinutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a jinutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal jinutil.Amount
		for _, deb := range details.Debits {
			debitTotal += deb.Amount
		}
		var outputTotal jinutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += jinutil.Amount(output.Value)
		}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := jinutil.Amount(output.Value).ToBTC()
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*jinutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*jinutil.Amount)
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr jinutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr jinutil.Address) (string, error) {
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *jinutil.WIF, bs *waddrmgr.BlockStamp,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]jinutil.Address{addr})
		if err != nil {
...
func (w *Wallet) NewAddress(account uint32, addrType waddrmgr.AddressType) (addr jinutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	addrType waddrmgr.AddressType) (jinutil.Address, error) {
	...
	// Request updates from jind for new transactions sent to this address.
	utilAddrs := make([]jinutil.Address, len(addrs))
...
func (w *Wallet) NewChangeAddress(account uint32, addrType waddrmgr.AddressType) (addr jinutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, addrType waddrmgr.AddressType) (jinutil.Address, error) {
...
// returning the total amount of jincoins received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr jinutil.Address, minConf int32) (jinutil.Amount, error) {
	var amount jinutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb jinutil.Amount) (*chainhash.Hash, error) {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*jinutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr jinutil.Address) (*btcec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr jinutil.Address) ([]byte, error) {
...
// in order to allow applications wrapping jinwallet to store app-specific data
