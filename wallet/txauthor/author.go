...
import (
	...
	"github.com/JinCoin/jinwallet/wallet/internal/txsizes"
)
...
// InputSourceError.
type InputSource func(target jinutil.Amount) (total jinutil.Amount, inputs []*wire.TxIn,
	inputValues []jinutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []jinutil.Amount
	TotalInput      jinutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb jinutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []jinutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := jinutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
