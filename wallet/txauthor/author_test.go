...
func p2pkhOutputs(amounts ...jinutil.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	f := func(target jinutil.Amount) (jinutil.Amount, []*wire.TxIn, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			u := unspents[0]
			unspents = unspents[1:]
			nextInput := wire.NewTxIn(&wire.OutPoint{}, nil)
			currentTotal += jinutil.Amount(u.Value)
...
func TestNewUnsignedTransaction(t *testing.T) {
	tests := []struct {
		...
		Outputs          []*wire.TxOut
		RelayFee         jinutil.Amount
		ChangeAmount     jinutil.Amount
