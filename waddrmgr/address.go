...
// style of jincoin addresses.  It additionally provides information about the
// script.
...
// Address returns the jinutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() jinutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*jinutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return jinutil.NewWIF(pk, a.manager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey *btcec.PublicKey,
	...
	if compressed {
		...
		pubKeyHash = jinutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address jinutil.Address
	var err error
	...
	case adtChainNestedWitness:
		...
		// backwards compatible to Jincoin nodes running 0.6.0 onwards, but
		...
		witAddr, err := jinutil.NewAddressWitnessPubKeyHash(pubKeyHash, m.chainParams)
		if err != nil {
			return nil, err
		}
		...
		address, err = jinutil.NewAddressScriptHash(witnessProgram, m.chainParams)
		if err != nil {
			return nil, err
		}
	case adtImport:
		...
		fallthrough
	case adtChain:
		address, err = jinutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams)
		if err != nil {
			return nil, err
		}
	case adtChainWitness:
		address, err = jinutil.NewAddressWitnessPubKeyHash(pubKeyHash, m.chainParams)
		if err != nil {
			return nil, err
		}
	}
...
type scriptAddress struct {
	manager         *Manager
	account         uint32
	address         *jinutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() jinutil.Address {
	return a.address
}
